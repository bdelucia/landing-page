import type { OpenWeatherConfig } from '$data/api-config.types';
...
import { weatherIconFromCondition, type WeatherDisplay } from '$lib/hooks/weather/weather';
...
export async function fetchCurrentWeather(): Promise<FetchWeatherResult> {
	if (!isOpenWeatherConfigured(apiSecrets)) {
		return { weather: null, error: null };
	}

	const { openWeather } = apiSecrets;
