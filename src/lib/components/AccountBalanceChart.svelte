<script lang="ts">
	...
	import type { CategoryBalanceSummary } from '$lib/account-balances';
	import {
		accountSeriesKey,
	...
	const totalBalance = $derived(
		detail.accounts.reduce((sum, account) => sum + account.balance, 0)
	);
	...
	const hoveredChartPoint = $derived.by(() => {
		...
		return (
			filteredChartData.find((point) => point.sortDate === tooltipPoint.sortDate) ?? null
		);
	});

	const hoveredCategoryTotals = $derived.by(() => {
		if (!hoveredChartPoint) return null;
		return categoryTotalsFromChartPoint(
			detail.accounts,
			itemLabelsByItemId,
			hoveredChartPoint
		);
...
<div class={className}>
	{#if detail.chartData.length === 0}
		<p class="px-5 py-6 text-sm text-muted-foreground" role="status">
		...
		<div class="px-2 py-4 sm:px-5 sm:py-5">
			<div class="relative h-[240px] w-full">
				...
				{#if hoverDatePosition}
					<div
						class="pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-full whitespace-nowrap text-[11px] font-medium text-muted-foreground"
			...
			>
				{#each CHART_TIME_RANGE_OPTIONS as option (option.key)}
					<button
						...
						class="rounded-md px-2 py-1 text-[11px] font-medium transition-colors data-[active=true]:bg-surface data-[active=true]:text-primary sm:px-2.5 sm:text-xs {activeTimeRange ===
						option.key
							? 'text-primary'
